...
1. CLI: `--ailoop-http` and `--ailoop-ws` (partial overrides merge with config when the other value comes from a file).
...
- `--with-api`: Also start Newton's HTTP API (`newton serve`) alongside the monitor (bind address and port: see `newton monitor --help`).
