...
"#;
    ...
    assert_eq!(config.project.template, Some("advanced-project".to_string()));
    ...
    assert_eq!(config.evaluator.test_command, Some("./scripts/comprehensive-tests.sh".to_string()));
    assert_eq!(config.evaluator.score_threshold, 88.5);
    assert!(!config.context.clear_after_use);
    assert_eq!(config.context.file, std::path::PathBuf::from(".custom/state/context.md"));
    assert_eq!(config.promise.file, std::path::PathBuf::from(".custom/state/promise.txt"));
}
...
"#;
    ...
    assert!(config.executor.auto_commit);
    assert_eq!(config.evaluator.test_command, Some("./env-tests.sh".to_string()));
    assert_eq!(config.evaluator.score_threshold, 95.0);
    assert!(!config.context.clear_after_use);
    assert_eq!(config.context.file, std::path::PathBuf::from(".env/context.md"));
    assert_eq!(config.promise.file, std::path::PathBuf::from(".env/promise.txt"));
...
"#;
    ...
    assert_eq!(config.context.file, std::path::PathBuf::from("custom/context.md")); // From file
