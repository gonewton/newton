...
pub struct BuiltinOperatorDeps {
    ...
    /// New: ailoop approver for GhOperator. Defaults to NoopApprover when None.
...
pub fn register_builtins_with_deps(
    ...
    let mut gh_operator = match deps.gh_runner {
        Some(runner) => gh::GhOperator::with_runner(runner),
        None => gh::GhOperator::new(),
    };
    if let Some(approver) = deps.gh_approver {
        gh_operator.set_approver(approver);
    }
