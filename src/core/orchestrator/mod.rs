...
impl OptimizationOrchestrator {
    ...
    pub async fn run_optimization(
        ...
        loop {
            ...
            self.reporter.report_info(&format!("Starting iteration {}", current_iteration + 1));
        ...
        self.reporter.report_info("Optimization completed successfully");
        Ok(execution)
    }
            ...
            match current_phase {
                IterationPhase::Evaluator => {
                    if let Some(evaluator_cmd) = &configuration.evaluator_cmd {
                        match self.execute_tool(evaluator_cmd, configuration, &execution.workspace_path).await {
                            Ok(result) => {
                                iteration.evaluator_result = Some(result);
                                if result.success {
                                    self.reporter.report_info("Evaluator completed successfully");
                                } else {
                ...
                IterationPhase::Advisor => {
                    if let Some(advisor_cmd) = &configuration.advisor_cmd {
                        match self.execute_tool(advisor_cmd, configuration, &execution.workspace_path).await {
                            Ok(result) => {
                ...
                IterationPhase::Executor => {
                    if let Some(executor_cmd) = &configuration.executor_cmd {
                        match self.execute_tool(executor_cmd, configuration, &execution.workspace_path).await {
                            Ok(result) => {
    ...
    async fn execute_tool(
        ...
        self.reporter.report_info(&format!("Executing tool: {}", cmd));
