...
fn parse_message(value: Value) -> Result<MonitorMessage, anyhow::Error> {
    ...
    let summary = build_summary(SummaryBuilderParams {
        kind: &kind,
        text: text.as_deref(),
...
fn build_summary(params: SummaryBuilderParams<'_>) -> String {
    match params.kind {
    ...
    value
        ...
        .or_else(|| {
            value
                .get("timestamp")
                .and_then(|v| v.as_i64())
                .and_then(|ts_num| Utc.timestamp_opt(ts_num, 0).single())
        })
}
