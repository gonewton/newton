...
pub use invocation::{LogInvocation, LogInvocationKind};
...
pub fn init(command: &LogInvocation, log_dir_override: Option<&Path>) -> Result<LoggingGuard> {
...
fn workspace_root_for_command(command: &LogInvocation) -> Result<Option<PathBuf>> {
    let candidate = match command.kind {
        LogInvocationKind::Monitor => match command.workspace_candidate.clone() {
            Some(p) => Some(p),
            None => {
        ...
        LogInvocationKind::Serve => None,
        _ => command.workspace_candidate.clone(),
    };
...
mod tests {
    ...
    fn make_run_command() -> LogInvocation {
    ...
    fn make_init_command() -> LogInvocation {
        LogInvocation::new(LogInvocationKind::Init, Some(PathBuf::from(".")))
    }

    fn make_monitor_command() -> LogInvocation {
        LogInvocation::new(LogInvocationKind::Monitor, None)
    }
