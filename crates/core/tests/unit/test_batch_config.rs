use newton_core::core::plan_queue_config::{find_workspace_root, parse_conf, PlanQueueConfig};
use std::fs;
...
#[test]
fn plan_queue_config_resolves_relative_project_root() {
    ...
    let config = PlanQueueConfig::load(workspace.path(), "proj").unwrap();
    assert_eq!(config.project_root, project_root);
}

#[test]
fn plan_queue_config_requires_workflow_file() {
    let workspace = TempDir::new().unwrap();
    let project_root = workspace.path().join("workspace-project");
    fs::create_dir_all(project_root.join(".newton")).unwrap();
    ...
    let result = PlanQueueConfig::load(workspace.path(), "proj");
