...
/// Build a [`newton_core::logging::LogInvocation`] from the parsed CLI command.
///
/// This is the bridge between the clap-derived `Command` enum (CLI-only) and
/// the clap-free invocation descriptor consumed by core logging/integrations.
pub fn log_invocation_from_command(command: &Command) -> newton_core::logging::LogInvocation {
    use newton_core::logging::{LogInvocation, LogInvocationKind};
    let (kind, ws): (LogInvocationKind, Option<std::path::PathBuf>) = match command {
        Command::Run(args) => (LogInvocationKind::Run, args.workspace.clone()),
        Command::Init(args) => (LogInvocationKind::Init, args.path.clone()),
        Command::Batch(args) => (LogInvocationKind::Batch, args.workspace.clone()),
        Command::Validate(args) => (
            LogInvocationKind::Validate,
            args.resolved_workflow_path()
        ...
        Command::Explain(args) => (LogInvocationKind::Explain, args.workspace.clone()),
        Command::Resume(args) => (LogInvocationKind::Resume, args.workspace.clone()),
        Command::Checkpoints(args) => {
            let ws = match &args.command {
                args::CheckpointCommand::List { workspace, .. } => workspace.clone(),
                args::CheckpointCommand::Clean { workspace, .. } => workspace.clone(),
            };
            (LogInvocationKind::Checkpoints, ws)
        }
        Command::Artifacts(args) => {
            let ws = match &args.command {
                args::ArtifactCommand::Clean { workspace, .. } => workspace.clone(),
            };
            (LogInvocationKind::Artifacts, ws)
        }
        Command::Webhook(args) => {
            let ws = match &args.command {
                args::WebhookCommand::Serve(s) => Some(s.workspace.clone()),
                args::WebhookCommand::Status(s) => Some(s.workspace.clone()),
            };
            (LogInvocationKind::Webhook, ws)
        }
        Command::Log(args) => {
            let ws = match &args.command {
                args::LogCommand::List { workspace, .. } => workspace.clone(),
                args::LogCommand::Show { workspace, .. } => workspace.clone(),
            };
            (LogInvocationKind::Log, ws)
        }
        Command::Monitor(_) => (LogInvocationKind::Monitor, None),
        Command::Serve(_) => (LogInvocationKind::Serve, None),
    };
    LogInvocation::new(kind, ws)
}
