//! The `BackendStore` trait: the storage contract the engine (`newton-core`)
//! consumes and `newton-backend` implements over sqlx/SQLite.
//!
//! Living in `newton-types` alongside the DTOs it speaks (see `models`) keeps
//! `newton-core` free of any concrete storage dependency (ADR-0015): the
//! engine depends on `Arc<dyn BackendStore>` and this crate only, never on
...
pub trait BackendStore: Send + Sync {
    ...
    async fn get_workflow_instance(&self, instance_id: &str) -> Result<WorkflowInstance, ApiError>;
    ...
    async fn upsert_workflow_instance(&self, instance: &WorkflowInstance) -> Result<(), ApiError>;

    async fn delete_workflow_instance(&self, instance_id: &str) -> Result<(), ApiError>;

    // ── NodeState ────────────────────────────────────────────────────────────────

    async fn get_node_state(&self, instance_id: &str, node_id: &str)
        -> Result<NodeState, ApiError>;

    async fn list_node_states_for_instance(
        &self,
        instance_id: &str,
    ) -> Result<Vec<NodeState>, ApiError>;

    /// Upserts on (instanceId, nodeId). Assigns a new UUID id on first insert.
    async fn upsert_node_state(&self, instance_id: &str, node: &NodeState) -> Result<(), ApiError>;
    ...
    async fn get_hil_event(&self, event_id: &str) -> Result<HilEvent, ApiError>;

    async fn list_hil_events_for_instance(
        &self,
        instance_id: &str,
    ) -> Result<Vec<HilEvent>, ApiError>;

    async fn list_hil_instances(&self) -> Result<Vec<String>, ApiError>;

    async fn insert_hil_event(&self, event: &HilEvent) -> Result<(), ApiError>;
