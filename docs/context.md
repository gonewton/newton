...
| **Discovery Process** | The automated pass that reads manifests/lockfiles across repos and produces `Detected` dependencies. It can only see *repo/package-level* dependencies; it structurally cannot see a **Module** in one **Repo** using an API from a Module in another. Those cross-repo Module edges must be supplied as `Declared`. | Scan, import |
